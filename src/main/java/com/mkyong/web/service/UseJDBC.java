...
// Загружаем нужный нам JDBC драйвер
        ...
        try (Connection con =
                     DriverManager.getConnection("jdbc:postgresql://localhost:5432/users", "postgres", "1")) {

// Создаем statement
            try (Statement stmnt = con.createStatement()) {
// Выполняем запрос к БД
                try (ResultSet rs = stmnt.executeQuery("SELECT * FROM users_table")) {// Перемщаем курсор по результам
                    while (rs.next()) {
// Извлекаем конкретные значения из ResultSet
                        ...
                        System.out.printf("id: %s, name: %s, surname: %s, patronimic: %s, pass:  %s, email: %s, phone: " +
                                        "%s\n",
                                id, name, surname, patronymic, pass,email,phone);
                    }
                }
            }
        }
